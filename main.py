...
# --- FUNGSI DIUBAH MENJADI SATU LANGKAH ---
def posting_ke_facebook(pesan: str, url_gambar: str):
    album_id = os.environ.get('FACEBOOK_ALBUM_ID')
    access_token = os.environ.get('FACEBOOK_ACCESS_TOKEN')
    
    print(f"🟢 Mengunggah foto ke album ID: {album_id} dan mempublikasikannya...")
    # Endpoint sekarang menargetkan album_id dan akan langsung mempublikasikan
    upload_url = f"https://graph.facebook.com/v20.0/{album_id}/photos"
    
    # Kita gabungkan caption langsung di sini
    upload_params = {
        'url': url_gambar,
        'caption': pesan, # Caption langsung dimasukkan di sini
        'access_token': access_token
    }
    
    response = requests.post(upload_url, params=upload_params)
    response.raise_for_status()
    print("✅ Foto berhasil diunggah ke album dan diposting ke timeline!")
...
{tags} {tag_umum}"""
        ...
        # Panggil fungsi posting yang sudah disederhanakan
